...
No Hermes container is declared because this repository has no approved Hermes image. Add a Compose `hermes` profile only after an exact trusted image and digest are configured; never invent an image name or bake credentials into it.
