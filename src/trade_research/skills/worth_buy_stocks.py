...
class WorthBuyStocksSkill:
    ...
    def analyze(self, instrument: InstrumentId, providers: ProviderRegistry) -> AnalystResult:
        ...
        alpha_factors, alpha_composite = _layer1_alpha_weighted(
            ...
            self._relative_strength_weight,
        ...
        verdict_codes = {
            ...
            _VERDICT_WATCH: 3,
            _VERDICT_NO: 1,
            _VERDICT_REDUCE_RISK: 2,
            _VERDICT_CANNOT_SCORE: 0,
        }
...
def _layer1_alpha_weighted(
    ...
    if mom is not None:
        ...
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.WORTH_BUY_MOMENTUM_SCORE,
                mom_score,
                observed_at,
                DerivedAlgorithm.WORTH_BUY_ALPHA_WEIGHTED,
                "momentum_12_1",
                prices=prices,
            )
        )
        ...
        for bm_sym, bm_prices in benchmarks.items():
            ...
            if rs is not None:
                ...
                factors.append(
                    _build_scoring_obs(
                        instrument,
                        MetricKind.WORTH_BUY_RELATIVE_STRENGTH,
                        rs,
                        observed_at,
                        DerivedAlgorithm.RELATIVE_STRENGTH,
                        f"vs_{bm_sym.lower()}",
                        prices=prices,
                    )
                )
    ...
    if er is not None:
        ...
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.WORTH_BUY_EFFICIENCY_SCORE,
                er * 100,
                observed_at,
                DerivedAlgorithm.KAUFMAN_EFFICIENCY,
                "30_day",
                prices=prices,
            )
        )
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_COMPOSITE,
            composite,
            observed_at,
            DerivedAlgorithm.WORTH_BUY_ALPHA_WEIGHTED,
            "composite_0_100",
...
def _relative_strength_score(target_closes: list[float], bench_closes: list[float]) -> float | None:
...
def _benchmark_instrument(label: str) -> InstrumentId:
    ...
    if normalized.isdigit() and len(normalized) == 6:
        market = (
            ...
            else ("BJSE" if normalized.startswith(("4", "8")) else "SZSE")
        )
...
def _layer2_risk_veto(
    ...
    if ma_short_val is not None and ma_long_val is not None:
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.SIMPLE_MOVING_AVERAGE,
                ma_short_val,
                observed_at,
                DerivedAlgorithm.SIMPLE_MOVING_AVERAGE,
                f"{ma_short}_observations",
                prices=prices,
            )
        )
    ...
    if dd is not None:
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.MAX_DRAWDOWN,
                dd * 100,
                observed_at,
                DerivedAlgorithm.EFFICIENCY_RATIO_CALC,
                "252_day",
                prices=prices,
            )
        )
    ...
    if weekly:
        ...
        if bearish_check.get("bearish") is True:
            factors.append(
                _build_scoring_obs(
                    instrument,
                    MetricKind.WEEKLY_BEARISH_ALIGNMENT,
                    1.0,
                    observed_at,
                    DerivedAlgorithm.WORTH_BUY_RISK_VETO,
                    "weekly_ma_bearish",
                    prices=prices,
                )
            )
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_RISK_VETO,
...
def _layer3_technical_confirmation(
    ...
    if len(closes) >= 15:
        ...
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.RELATIVE_STRENGTH_INDEX_14,
                rsi14,
                observed_at,
                DerivedAlgorithm.WILDER_RSI,
                "14_observations",
                prices=prices,
            )
        )
        ...
        if k_val is not None:
            factors.append(
                _build_scoring_obs(
                    ...
                    k_val,
                    observed_at,
                    DerivedAlgorithm.KDJ_CALCULATION,
                    f"{kdj_n}_{kdj_k}_{kdj_d}",
                    prices=prices,
                )
            )
    ...
    if adx_val is not None:
        factors.append(
            _build_scoring_obs(
                ...
                adx_val,
                observed_at,
                DerivedAlgorithm.ADX_CALCULATION,
                f"{adx_n}_observations",
                prices=prices,
            )
        )
        if adx_val < 20:
    ...
    if ud_ratio is not None:
        factors.append(
            _build_scoring_obs(
                instrument,
                MetricKind.UP_DOWN_VOLUME_RATIO,
                ud_ratio,
                observed_at,
                DerivedAlgorithm.EFFICIENCY_RATIO_CALC,
                "10_day",
...
def _layer4_entry_timing(
    ...
    ohlcv_prices = [
        p for p in prices if all(v is not None for v in (p.open, p.high, p.low, p.volume))
    ]
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_ENTRY_PRICE,
            entry_price,
            observed_at,
            DerivedAlgorithm.WORTH_BUY_ENTRY_TIMING,
            "current_close",
            prices=prices,
        )
    )
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_STOP_PRICE,
            round(stop_price, 2),
            observed_at,
            DerivedAlgorithm.WORTH_BUY_ENTRY_TIMING,
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_TARGET_PRICE,
            recent_high,
            observed_at,
            DerivedAlgorithm.WORTH_BUY_ENTRY_TIMING,
            "52_week_high",
    ...
    factors.append(
        _build_scoring_obs(
            instrument,
            MetricKind.WORTH_BUY_ENTRY_CLASSIFICATION,
            float(entry_class),
            observed_at,
            DerivedAlgorithm.WORTH_BUY_ENTRY_TIMING,
...
def _classify_entry(
    ...
    if ma20 is not None and last_close > ma20:
        ...
        if signal_vals and macd_vals[-1] > signal_vals[-1] and adx_val is not None and adx_val > 25:
    ...
    risk_checks = (
        ReportCheck(
            ...
            status=(
                ...
                if risk_flags & {"bearish_ma_alignment", "below_ma_short"}
                else "pass"
            ),
        ...
        ReportCheck(
            ...
            status=(
                ...
                if "severe_drawdown" in risk_flags
                else "warning"
                if "moderate_drawdown" in risk_flags
                else "pass"
            ),
            value=drawdown,
        ),
...
def _empty_result(instrument: InstrumentId, name: str, missing: list[str]) -> AnalystResult:
    return AnalystResult(
        analyst=name,
        instrument=instrument,
